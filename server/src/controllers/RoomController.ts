...
const upload = multer({
  storage: multerS3({
    ...
    key: (req, file, cb) =>
      cb(null, `roomTypesImage/${Date.now()}-${file.originalname}`),
  }),
});
      ...
      rooms.map(async (room) => {
        ...
        if (
          ...
          (room.images[0].startsWith("http://") ||
            room.images[0].startsWith("https://"))
        ) {
          return room;
        }
...
export const updateRoomCleaningDate: RequestHandler = async (
  req,
  res,
  next
) => {
  try {
    ...
    if (!cleaningDate) {
      res
        ...
        .json({ message: "Cleaning date is required" });
      return;
    }
    ...
    res
      .status(STATUS.OK)
      .json({ message: "Room cleaning date updated", room: updatedRoom });
  } catch (e) {
    next(e);
  }
};
  ...
  try {
    ...
    const updatedRoom = await prisma.room.update({
      ...
      data: {
        ...
        ...(smoking !== undefined && {
          smoking: smoking === "Smoking" || smoking === true,
        }),
...
export const updateRoomType: RequestHandler = async (req, res, next) => {
  try {
    ...
    const { name, description, basePrice, capacity, maxOccupancy, amenities } =
      parseResult.data;
...
export const getRoomIdByRoomNumber: RequestHandler = async (req, res, next) => {
  try {
    ...
    res.status(STATUS.OK).json({ id: room.id });
  } catch (e) {
    next(e);
  }
};
